...
import controller.newInterpreter.{CombinedExpression, CommandType, InterpretedCommand, InterpretedExpression, InterpretedUnidentified}
...
case class CommandValidator(orig: String, gsm: GameStateManager) extends IValidator:
  ...
  override def validate(expr: Vector[InterpretedExpression]): Either[IValidator, Option[ICommand]] =
  ...
  private def handleCommand(cT: CommandType, rest: Vector[InterpretedExpression]): IValidator | ICommand =
    cT match
  ...
  private def handleTerminalCommand(cT: CommandType, rest: Vector[InterpretedExpression]): ICommand =
    cT match
