...
case class Fleet(name: String = "Battlegroup-" + Random().between(1,1000),
                 fleetComponents: Vector[Component] = Vector(),
  ...
  def remove(tbr: Vector[Component]): Option[IFleet] =
    if fleetComponents.intersect(tbr).nonEmpty then Some(this.copy(fleetComponents = fleetComponents.diff(tbr)))
    else None
  def units: Vector[IUnit] = fleetComponents.asInstanceOf[Vector[IUnit]]
  ...
  override def split(): (IFleet, IFleet) =
    val (f1, f2) = fleetComponents.splitAt(fleetComponents.length / 2)
  ...
  override def extCopy(name: String, fleetComponents: Vector[Component], location: ISector, moveVector: IMoveVector): IFleet =
    this.copy(name, fleetComponents, location, moveVector)

  override def merge(f: Fleet): IFleet =
    this.copy(fleetComponents = fleetComponents.appendedAll(f.fleetComponents))