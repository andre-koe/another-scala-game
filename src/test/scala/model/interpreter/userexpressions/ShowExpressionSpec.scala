...
import controller.command.commands.{InvalidCommand, UserAcceptCommand, UserDeclineCommand}
...
class ShowExpressionSpec extends AnyWordSpec {
  
  "The ShowExpressionSpec" when {
    val gameStateManager: GameStateManager = GameStateManager()

    "given the inputs  should return an UserResponseCommand" in {
      val input1 = List("y")
      val input2 = List("yes")
    ...
    "given any other inputs should return an InvalidCommand" in {
      ...
      userResponseExp1.interpret(gameStateManager) shouldBe a[InvalidCommand]
      userResponseExp2.interpret(gameStateManager) shouldBe a[InvalidCommand]
    }
